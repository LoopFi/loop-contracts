...
contract CombinedAggregatorV3Oracle {
    ...
    function getAggregatorData() public view returns (uint256, uint256) {
        ...
        if (!isValid) revert CombinedAggregatorV3Oracle__invalidAggregatorValue(address(aggregator));
        ...
        returns (uint80 roundId, int256 answer, uint256 startedAt, uint256 updatedAt, uint80 answeredInRound)
    {
        ...
        if (!isValid) revert CombinedAggregatorV3Oracle__invalidAggregatorValue(address(aggregator2));
    ...
    function _fetchAndValidate() internal view returns (bool isValid, uint256 price) {
        try aggregator2.latestRoundData() returns (
            uint80,
            int256 answer,
            uint256 /*startedAt*/,
            uint256 updatedAt,
            ...
            isValid = (answer > 0 && block.timestamp - updatedAt <= aggregator2Heartbeat);
            return (isValid, wdiv(uint256(answer), aggregator2Scale));
        } catch {

...
contract SwapAction is TransferAction {
    ...
    function pendleJoin(address recipient, uint256 minOut, bytes memory data) internal returns (uint256 netLpOut) {
        ...
        if (input.tokenIn != address(0)) {
            // this balance of overwrite is breaking the increase lever flow because this swap happens before the pool join and there is no token left to join with.
            // Opening a new position can avoid this issue by not using the pool join action and relying on this swap action to join the full amount, but this breaks the ability to add collateral through increase lever without needing to borrow anything.

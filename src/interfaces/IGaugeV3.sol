...
interface IGaugeV3 is IGaugeV3Events, IVotingContractV3, IVersion {
    ...
    function userTokenVotes(address user, address token) external view returns (uint96 votesLpSide, uint96 votesCaSide);

    function quotaRateParams(
        address token
    ) external view returns (uint16 minRate, uint16 maxRate, uint96 totalVotesLpSide, uint96 totalVotesCaSide);
    ...
    function addQuotaToken(address token, uint16 minRate, uint16 maxRate, address vault) external;

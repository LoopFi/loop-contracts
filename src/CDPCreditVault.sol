...
contract CDPCreditVault is ERC4626, ERC20Permit, ACLNonReentrantTrait, ContractsRegisterTrait, IPoolV3 {
    ...
    /// @notice Global accounting module for credit and debt in the system
    address public immutable override cdm;
    ...
    /// @dev User has to set allowance for the Pool to transfer Underlying tokens
    /// @param user Address of the user
    /// @param amount Amount of Underlying tokens to be redeemed for internal credit [wad]
    function enter(address user, uint256 amount) 
        external 
        override 
        nonReentrant
    {
        ICDM(cdm).modifyBalance(address(this), user, amount);
        IERC20(underlyingToken).safeTransferFrom({from: msg.sender, to: address(this), value: amount}); // U:[LP-6,7]
        emit Enter(user, amount);
    }

    /// @notice Redeems internal credit for Underlying tokens
    /// @dev User has to grant the delegate of transferring credit to the Pool
    /// @param user Address of the user
    /// @param amount Amount of credit to be redeemed for Underlying tokens
    function exit(address user, uint256 amount) 
        external 
        override 
        whenNotPaused
        nonReentrant
    {
        ICDM(cdm).modifyBalance(msg.sender, address(this), amount);
        IERC20(underlyingToken).safeTransfer({to: user, value: amount}); // U:[LP-8,9]

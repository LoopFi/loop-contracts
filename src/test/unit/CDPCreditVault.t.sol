...
import {IPoolV3Events} from "@gearbox-protocol/core-v3/contracts/interfaces/IPoolV3.sol";
...
import {AddressProviderV3ACLMock} from "@gearbox-protocol/core-v3/contracts/test/mocks/core/AddressProviderV3ACLMock.sol";

import {ERC20FeeMock} from "@gearbox-protocol/core-v3/contracts/test/mocks/token/ERC20FeeMock.sol";

import {CDPCreditVaultHarness} from "./helper/CDPCreditVaultHarness.sol";
...
/// @title CDPCreditVaultTest unit test
/// @dev Based on the unit tests by Gearbox Protocol
/// @notice U:[LP]: Unit tests for lending pool
contract CDPCreditVaultTest is TestBase, TestHelper, IPoolV3Events, IERC4626Events {
    CDPCreditVaultHarness pool;
    ...
    function _setupPool() internal {
        pool = new CDPCreditVaultHarness({
        ...
        cdm.setParameter(address(pool), "debtCeiling", 100_000_000 ether);
    ...
    // ----------------- //
    function test_U_LP_12A_enter() public {
        // prepare assets
        _prepareAssets(address(user), 100 ether);

        // deposit underlying for credit
        vm.startPrank(address(user));
        pool.enter(address(user), 100 ether);

        assertEq(underlying.balanceOf(address(user)), 0);
        assertEq(credit(address(user)), 100 ether);
    }

    function test_U_LP_12B_enter_to_user() public {
        _prepareAssets(address(this), 100 ether);
        assertEq(credit(user), 0);

        // deposit underlying for credit
        pool.enter(user, 100 ether);

        // check enter to user was successful
        assertEq(stablecoin.balanceOf(address(user)), 0 ether);
    ...
    function test_U_LP_12C_enter_fails_with_insufficient_underlier() public {
        _prepareAssets(address(user), 100 ether);

        // deposit underlying for credit
        vm.expectRevert();
        vm.startPrank(address(user));
        pool.enter(address(user), 101 ether);
    }

    function test_U_LP_12D_enter_fails_when_paused() public {
        _prepareAssets(address(user), 100 ether);
    ...
    function test_U_LP_13A_exit() public {
        createCredit(address(user), 100 ether);
        deal({token: address(underlying), to: address(pool), give: 100 ether, adjust: true});

        // withdraw underlying for credit
        vm.startPrank(address(user));
        cdm.modifyPermission(address(pool), true);
        pool.exit(address(user), 100 ether);
        vm.stopPrank();

        assertEq(underlying.balanceOf(address(user)), 100 ether);
        assertEq(credit(address(user)), 0 ether);
    }

    function test_U_LP_13B_exit_to_user() public {
        createCredit(address(this), 100 ether);
        deal({token: address(underlying), to: address(pool), give: 100 ether, adjust: true});

        // mint stablecoin from credit
        cdm.modifyPermission(address(pool), true);
        pool.exit(user, 100 ether);
        ...
        // check no unexpected left over amounts
        assertEq(underlying.balanceOf(address(this)), 0 ether);
        assertEq(credit(user), 0 ether);
    }


    function test_U_LP_13C_exit_fails_with_insufficient_underlier() public {
        createCredit(address(user), 100 ether);
    ...
    function test_U_LP_13D_exit_fails_with_insufficient_credit() public {
        createCredit(address(user), 100 ether);
        deal({token: address(underlying), to: address(pool), give: 200 ether, adjust: true});
        ...
        pool.exit(address(user), 110 ether);
    }

    function test_U_LP_13E_exit_fails_when_paused() public {
        createCredit(address(this), 100 ether);
        deal({token: address(underlying), to: address(pool), give: 200 ether, adjust: true});
